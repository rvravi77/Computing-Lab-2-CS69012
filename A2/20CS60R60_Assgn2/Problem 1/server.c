...
//to cho pfirst N characters from string
...
{	
    ...
    int client_socket,client_index=1,x;         //socket of client , index number of client , fork variable
	...
	if(listen(server_socket, 15) < 0) // connection waiting at particular time
    {
        perror("Listen Error\n");
        exit(1);
    }
    ...
    while(1){
		...
		int x=fork();
		if(x==0){
			//If Child Process then do evaluation
			break;
		}
		else{
			client_index++;
    ...
    while(1){
		memset(str,'\0',WORD);
        memset(temp,'\0',WORD);
		if(read(client_socket,str,WORD)<0){
            perror("READ :");
            exit(1);
        }
        ...
        int count = 0; 
        for (int i = 0; i < strlen(str); i++) {   //removing white spaces from string
            if (str[i] != ' ') 
                str[count++] = str[i]; 
            }
        str[count] = '\0'; 
        buffer=in_to_post(str);                   //function  call to convert infix to postfix
        if(buffer[0]==' '){                       //remove first white space from string
            chopN(buffer,1);
            buffer[strlen(buffer)]='\0';
        }
        buffer2=eval_post(buffer);             //function call to evaluate postfix expression
        if(strcasecmp(buffer2,"ERROR")==0)
            buffer="ERROR";
        sprintf(temp,"%s , %s\n",buffer,buffer2); //string formatting for sending reply back
...
char *eval_post(char *str)
{
	...
	for(int i=0;i<strlen(str);i++){
		...
		c=str[i];
		if(isdigit(c)!=0){                  //if current char is digit
			double n2=0;
			token=0;
            int cnt=0;
        	...
        	if(str[i]=='.'){                 //convert the floating point number
                i++;
                while (i < strlen(str) && isdigit(str[i])){   
                    n2 = n2 * 10 + str[i++] - '0';
                    cnt++;
